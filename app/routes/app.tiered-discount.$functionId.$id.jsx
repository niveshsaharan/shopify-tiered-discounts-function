...
export const loader =  async ({ params, request }) => {
  ...
  return ({params, discount: responseJson});
}
...
export default function EditDiscount(props) {
  ...
  const discountData = useLoaderData()
...
console.log(discountDetail)
  ...
  const dbTiers = JSON.parse(discountDetail?.metafield?.value || JSON.stringify({tiers: [], type: 'tiered',
    message: 'Congratulations! You get {{percentage}} % off your order!',
    next_message: `Spend {{remaining}} more and get {{percentage}} % off.`}));

  const tiers = useDynamicList(dbTiers.tiers, emptyTierFactory);
  ...
  const {
    ...
    fields: {
      discountTitle: useField(discountDetail?.discount?.title),
      discountMethod: useField(DiscountMethod.Automatic),
      discountCode: useField(""),
      combinesWith: useField(discountDetail?.discount?.combinesWith || {
      ...
      startDate: useField(discountDetail?.discount?.startsAt ? new Date(discountDetail?.discount?.startsAt) : new Date()),
      endDate: useField(discountDetail?.discount?.endsAt ? new Date(discountDetail?.discount?.endsAt) : null),
      dynamicLists: {
        tiers,
      },
      configuration: {
        ...
        type: useField(dbTiers.type),
        message: useField(dbTiers.message),
        next_message: useField(dbTiers.next_message),
      },
    },
    onSubmit: async (form) => {
      const discount = {
